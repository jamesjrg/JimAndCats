...
[<Tests>]
let userCreationTests =
    let Expect = ExpectAfterCreateCommand (CommandHandling.createUser createGuid1 createEpoch id)    
    let ExpectBadRequest = Expect (Failure (BadRequest "any string will do"))

    testList "User creation tests"
        [
            testCase "Should be able to create a user" (fun () ->  
                { CreateUser.Name="Bob Holness"; Email="bob.holness@itv.com"; Password="p4ssw0rd" }
                |> Expect (Success { UserCreated.Id = guid1; Name = Username "Bob Holness"; Email= EmailAddress "bob.holness@itv.com"; PasswordHash=PasswordHash "p4ssw0rd"; CreationTime = epoch } ))

            testCase "Should not be able to create a user with too short a username" (fun () -> 
                { CreateUser.Name="Bob"; Email="bob.holness@itv.com"; Password="p4ssw0rd" }
                |> ExpectBadRequest)

            testCase "Should not be able to create a user with large whitespace username" (fun () ->    
                { CreateUser.Name="                 "; Email="bob.holness@itv.com"; Password="p4ssw0rd" }
                |> ExpectBadRequest)

            testCase "Should not be able to create a user with invalid email address" (fun () -> 
                { CreateUser.Name="Bob Holness"; Email="bob.holnessitv.com"; Password="p4ssw0rd" }
                |> ExpectBadRequest)
        ]
