...
let queryTests =
    testList "Query web API tests"
        ...
        testCase "Should be able to fetch a user" (fun () ->
            let actual = get getWebServerWithAUser "/users/3C71C09A-2902-4682-B8AB-663432C8867B" bobCredentials contentString
        ...
        testCase "Should get unauthorized for non-existent user if requester is not an admin" (fun () ->
            let actual = get getWebServerWithNoEvents "/users/3C71C09A-2902-4682-B8AB-663432C8867B" bobCredentials statusCode
            
            actual =? HttpStatusCode.Unauthorized)

        testCase "Should get 404 for get request to incorrect url" (fun () ->
            let actual = get getWebServerWithNoEvents "/flibbles" bobCredentials statusCode
